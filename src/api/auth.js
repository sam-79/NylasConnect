...
export const syncCalenderData = (params) => {
    return new Promise(async (resolve, reject) => {
        console.log('🚀 ~ file: auth.js ~ SyncCalenderdata ~ params:', params);
        ...
        try {
            const response = await fetch(`${params.hostname}/api/users/sync_calendars`, requestOptions);
            const jsonData = await response.json();
            ...
            if (response.status === 200 && jsonData.status == "success") {
                ...
                // Handle status 422 error
                reject({ status: 422, message: jsonData });
            } else {
                // Handle other errors
                reject({ status: response.status, message: jsonData });
            }
        } catch (error) {
...
export const syncGrantData = createAsyncThunk('syncGrants', async (params, thunkApi) => {
...
// export const syncCalenderData = createAsyncThunk('syncGrants', async (params, thunkApi) => {
